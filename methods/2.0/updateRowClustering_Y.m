function [Cy] = updateRowClustering_Y(q, tilde_q, q0, tilde_q0, Cy, beta)
...
parfor i = 1:size(tilde_q,2)
  tilde_qZty(:,i) = accumarray(Cy, tilde_qZy(:,i));
end

parfor i = 1:size(tilde_q0,2)
...
for rc = 1:size(tilde_qZty, 1)
  parfor r = 1:size(qZy, 1)
    temp(rc, r) = KLDiv(qZy(r,:), tilde_qZty(rc,:)) + beta * KLDiv(q0Zy(r,:), tilde_q0Zty(rc,:));
  end
end
