...
write.table(VariableFeatures(S),"../../data/ex14/FS.csv",row.names=T,col.names=TRUE,sep=",")
# S = ScaleData(S)
# S = RunPCA(S)
...
write.table(lS,"../../data/ex14/lS.csv",row.names=T,col.names=TRUE,sep=",")
TU = readRDS("../../data/ex1/TU.rds")
TU = NormalizeData(TU)
TU = FindVariableFeatures(TU, selection.method = "vst", nfeatures = 6000)
write.table(VariableFeatures(TU),"../../data/ex14/FT.csv",row.names=T,col.names=TRUE,sep=",")
# TU = ScaleData(TU)
# TU = RunPCA(TU)
...
write.table(lT,"../../data/ex14/lT.csv",row.names=T,col.names=TRUE,sep=",")
SF = read.csv("../../data/ex14/FS.csv")
SF = SF[[1]]
TF = read.csv("../../data/ex14/FT.csv")
TF = TF[[1]]
sum(SF %in% TF)
Feature = SF[SF %in% TF]
write.table(Feature,"../../data/ex14/F.csv",row.names=T,col.names=TRUE,sep=",")
mS = S@assays$RNA@data[Feature,]
ct = unique(S@meta.data$cell_type)
for (i in ct) {
  m = mS[, which(S@meta.data$cell_type == i)]
  mS[, which(S@meta.data$cell_type == i)] = m/sd(m)
}
write.csv(mS,"../../data/ex14/mS.csv")
mT = TU@assays$RNA@data[Feature,]
write.csv(mT,"../../data/ex14/mT.csv")
mU = TU@assays$ATAC@counts
ct = rowSums(mU)
UF = names(sort(ct, decreasing = T)[1:2000])
mU = TU@assays$ATAC@counts[UF,]
Matrix::writeMM(mU,"../../data/ex14/mU.csv")
