function [Cx, Cy, Cz, Cu, cluster_p, cluster_q, cluster_q0, cluster_pt] = coupleCoC_plus(p,q,q0,nrowcluster1,nrowcluster2,ncolcluster,ncolcluster0,iter,lambda,beta,gamma,Cx_truth)
%p: preprocessed S;
%q: preprocessed T;
%q0: preprocessed U;
...
%   Coef = updateMatchCoef(cluster_p, cluster_q, ind_X, ind_Y, gamma);
  ...
  Cx = updateRowClustering_X(p, tilde_p, Cx, gamma, Cx_truth);

  % update Cy(Y) based on q, tilde_q, q0, tilde_q0, Coef;
  Cy = updateRowClustering_Y(q, tilde_q, q0, tilde_q0, Cy, beta);
  
  % updata Cz0(Z0) based on q0, tilde_q0
  Cu= updateColClustering_Zu(q0, tilde_q0, Cu);

  %[Cz, dist] = updateColClustering_cocpc(p, tilde_p, Cz);
  % update Cz(Z) based on p, q, tilde_p, tilde_q
  Cz= updateColClustering_Z(p, q, tilde_p, tilde_q, Cz, lambda);
...
[tilde_pt, cluster_pt] = updateTildep_plus(p, Cx_truth, Cz);
